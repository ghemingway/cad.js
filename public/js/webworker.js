...
/***/ function(module, exports) {
	...
	var TYSON = (function () {
	    ...
	    var TYSON = function TYSON(buffer) {
	        ...
	        var values = [];
	        var imports = {
	            pushNull: function pushNull() {
	                values.push(null);
	            },
	            pushTrue: function pushTrue() {
	                values.push(true);
	            },
	            pushFalse: function pushFalse() {
	                values.push(false);
	            },
	            pushInt: function pushInt(val) {
	                values.push(val);
	            },
	            pushFloat32: function pushFloat32() {
	                var pos = core.getPos();
	                var val = view.getFloat32(pos);
	                values.push(val);
	                core.setPos(pos + 4);
	            },
	            pushFloat64: function pushFloat64() {
	                var pos = core.getPos();
	                var val = view.getFloat64(pos);
	                values.push(val);
	                core.setPos(pos + 8);
	            },
	            pushDecimal: function pushDecimal(size) {
	            ...
	            pushString: function pushString(size) {
	                var start = core.getPos();
	                var stop = start + size;
	                var val = "";
	                for (var i = start; i < stop; i++) {
	            ...
	            newObject: function newObject(count) {
	                var val = {};
	            ...
	            newTypedArray: function newTypedArray(count, arrayType) {
	                var start = core.getPos();
	                var bytes = 1;
	                switch (arrayType) {
	                    case 66:
	            ...
	            newArray: function newArray(count) {
	                var val = [];
	                for (var i = count - 1; i >= 0; i--) {
	                    var v = values.pop();
	                    val[i] = v;
